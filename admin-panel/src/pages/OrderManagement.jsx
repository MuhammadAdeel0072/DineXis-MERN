  ...
  return (
    ...
    >
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <header>
        <h1 className="text-4xl font-serif font-black text-soft-white tracking-tighter">Order <span className="text-gold">Logistics</span></h1>
        <p className="text-soft-white/50 mt-2 uppercase text-[10px] font-bold tracking-[0.2em]">AK-7 REST LIVE OPERATION TRACKER</p>
      </header>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/5 text-soft-white/40 text-[10px] uppercase tracking-[0.2em]">
                <th className="px-8 py-6 font-bold">Patron & Order ID</th>
                <th className="px-8 py-6 font-bold">Investment</th>
                <th className="px-8 py-6 font-bold">Current State</th>
                <th className="px-8 py-6 font-bold text-right">Command</th>
              </tr>
            </thead>
                  ...
                  {filteredOrders.map((order) => (
                    ...
                    >
                      <td className="px-8 py-8">
                        <div>
                          <p className="font-bold text-soft-white text-lg">#{order.orderNumber || order._id.slice(-6).toUpperCase()}</p>
                          <p className="text-soft-white/40 text-sm mt-1">{order.user?.firstName} {order.user?.lastName || 'Guest Patron'}</p>
                          <p className="text-[10px] text-gold/50 mt-2 uppercase tracking-tighter">{new Date(order.createdAt).toLocaleString()}</p>
                        </div>
                      </td>
                      <td className="px-8 py-8 font-serif font-bold text-soft-white text-lg">
