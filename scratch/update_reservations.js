...
const filePath = path.join(__dirname, '../client/src/pages/Reservations.jsx');
...
if (returnStartIndex !== -1 && returnEndIndex !== -1) {
        ...
        <div className="container mx-auto px-4 md:px-6 py-8 md:py-12 max-w-5xl">
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'book' ? (
                    ...
                    >
                        <div className="bg-white/[0.02] backdrop-blur-3xl border border-white/5 p-6 md:p-10 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl">
                            ...
                            <form onSubmit={handleBooking} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            min={min}
                                            max={max}
                                            className="w-full bg-black/20 border border-white/10 rounded-2xl p-3.5 focus:border-gold outline-none text-white transition-all [color-scheme:dark]"
                                            value={formData.date}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select
                                            className="w-full bg-black/20 border border-white/10 rounded-2xl p-3.5 focus:border-gold outline-none text-white transition-all [&>option]:bg-[#1a1a1a] [&>option]:text-white"
                                            value={formData.time}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            max="20"
                                            required
                                            className="w-full bg-black/20 border border-white/10 rounded-2xl p-3.5 focus:border-gold outline-none text-white transition-all"
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            className={\`w-full bg-black/20 border rounded-2xl p-3.5 focus:border-gold outline-none text-white transition-all placeholder:text-gray-600 \${phoneError ? 'border-red-500/60' : 'border-white/10'}\`}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-black/20 border border-white/10 rounded-2xl p-3.5 focus:border-gold outline-none text-white transition-all placeholder:text-gray-600"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-black/20 border border-white/10 rounded-2xl p-4 focus:border-gold outline-none text-white transition-all placeholder:text-gray-600 resize-none"
                                ...
                                <div className="bg-gradient-to-r from-gold/10 to-transparent border border-gold/20 rounded-2xl p-5 relative overflow-hidden group">
                                    <div className="absolute top-0 left-0 w-1 h-full bg-gold"></div>
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                                        <div className="space-y-2 flex-1">
                                            <div className="flex items-center gap-2 text-gold font-bold text-xs uppercase tracking-widest mb-1">
                                                <AlertCircle className="w-4 h-4" /> Advance Payment (Rs. 1,000)
                                            </div>
                                            <p className="text-gray-400 text-[11px] leading-relaxed">
                                                To confirm booking, please send <strong className="text-white">Rs. 1,000</strong> to:
                                            </p>
                                            {formData.paymentMethod === 'EasyPaisa' && (
                                                <div className="text-lg font-black text-gold tracking-widest font-mono">
                                        ...
                                        <div className="flex-1 w-full flex flex-col gap-3">
                                            ...
                                            <button 
                                                ...
                                                className={\`w-full py-3.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all \${hasPaid ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gold/10 hover:bg-gold/20 text-gold border border-gold/20'}\`}
                                            >
                                                {hasPaid ? '✓ Verified' : 'Verify Payment'}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button
                                    ...
                                    className="w-full bg-gold hover:bg-gold/90 text-charcoal font-black py-4.5 rounded-2xl flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(212,175,55,0.2)] hover:shadow-[0_0_40px_rgba(212,175,55,0.4)] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-4 text-sm tracking-wider uppercase"
                                >
                                    {isBooking ? <Loader2 className="w-5 h-5 animate-spin" /> : hasPaid ? 'Confirm Reservation' : 'Complete Payment First'}
                                </button>
                            </form>
                        </div>
                    </motion.div>
    ...
    content = content.substring(0, returnStartIndex) + newReturn + content.substring(returnEndIndex + 6);
    fs.writeFileSync(filePath, content, 'utf8');
    console.log('Successfully updated Reservations.jsx');
} else {
